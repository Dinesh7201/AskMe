...
const AskScreen = ({ Email,navigation }) => {
    const email = Email;
    ...
    const writeQuery = async () => {
        try {
            const write = await database().ref(`queries/${length}`).set({
                    name: name,
                    message: query,
                    curretTime: moment().utcOffset('+05:30').format('MMMM Do YYYY, h:mm:ss a'),
                });
                navigation.navigate('commu');
            console.log(write);
        }
        catch (e) {
            console.log(e);
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={{ backgroundColor: "white", marginVertical: 10, borderRadius: 15, marginHorizontal: 10 }}>
                <TextInput style={{ textAlign: "center", fontSize: 30, color: "black", fontFamily: "Futura-CondensedLight" }} placeholder='Type your Question here' placeholderTextColor="blue" onChangeText={(value) => setQuery(value)} />
            </View>
            <TouchableOpacity onPress={() => { writeQuery() }}>
                <View style={{ backgroundColor: "white", marginVertical: 10, borderRadius: 15, marginHorizontal: 100 }}>
                    <Text style={{ color: "purple", fontSize: 30, textAlign: "center", marginVertical: 10, marginHorizontal: 10 }}>Post</Text>
                </View>
            </TouchableOpacity>
        </View>
    );
